...
# 开机自启（仅需一次）
pm2 startup   # 或手动将 pm2 resurrect 加入开机启动项
pm2 save
```
...
| 字段 | 说明 |
|------|------|
| `scheduler.startTime` / `endTime` | 每日执行窗口 |
| `scheduler.intervalMinutes` | 窗口内执行间隔（分钟） |
| `command.executable` / `args` | 定时触发的命令及参数 |
| `monitor.intervalMinutes` | 额度采集间隔（分钟） |
| `monitor.agyTimeoutMs` | 采集 HTTP 超时 |
| `web.port` / `web.host` | 仪表盘监听地址 |
...
**技术栈：** [Bun](https://bun.sh/) · [Elysia](https://elysiajs.com/) · SQLite · Chart.js · Vanilla JS
